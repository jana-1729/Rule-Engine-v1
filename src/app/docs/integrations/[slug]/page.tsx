...
export default async function IntegrationDocPage({
  ...
  let content: string;
  try {
    const filePath = join(process.cwd(), 'docs', 'integrations', `${params.slug}.md`);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-8 flex items-center justify-between">
          <span className="text-sm font-medium text-blue-900">Related Resources:</span>
          <div className="flex items-center gap-4">
            <Link 
              href={`/docs/blogs/${params.slug}-automation-guide`}
            ...
            {session && (
              <Link 
                href={`/dashboard/integrations/${params.slug}`}
                className="text-sm text-blue-600 hover:text-blue-700 flex items-center gap-1"
              >
        ...
        <div className="mt-12 bg-gradient-to-r from-blue-600 to-purple-600 rounded-2xl p-8 text-center text-white">
          <h2 className="text-2xl font-bold mb-3">Ready to integrate {title.replace(' Integration', '')}?</h2>
...
// Generate static params for all integration docs
